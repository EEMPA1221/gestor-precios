...
export default function CompararProveedoresPage() {
  ...
  useEffect(() => {
    // Cargamos ACTIVE y PENDING para no perder listas recién importadas
    Promise.all([
      fetch('/api/listas?status=ACTIVE').then(r => r.json()),
      fetch('/api/listas?status=PENDING').then(r => r.json()),
    ]).then(([active, pending]) => {
      const all = [...(Array.isArray(active) ? active : []), ...(Array.isArray(pending) ? pending : [])]
      // Deduplicar por id
  ...
  async function runComparison() {
    ...
    try {
      const responses = await Promise.all(
        selectedIds.map(id =>
          fetch(`/api/listas/${id}`).then(r => r.json())
        )
      )
      ...
      for (const list of responses) {
        ...
        for (const item of list.items) {
          ...
          if (!map.has(key)) {
            map.set(key, {
              key,
              internalName: item.originalName,
              prices: {},
              ivaRates: {},
              bestListId: null,
              bestPrice: null,
      ...
      result.sort((a, b) => a.internalName.localeCompare(b.internalName))
      setRows(result)
    } finally {
      setLoading(false)
    }
  ...
  const filtered = useMemo(() => {
    ...
    return r
  }, [rows, search, showOnlyDiff, selectedIds])
  ...
  function toggleRow(row: CompRow) {
    ...
    const items: ListItem[] = rows
      .filter(r => next.has(r.key) && r.bestListId && r.bestPrice != null)
      .map(r => ({
        id: r.key,
        listId: r.bestListId!,
  ...
  function toggleAllRows() {
    if (checkedRows.size === filtered.length) {
      ...
      const ids = new Set(filtered.map(r => r.key))
      setCheckedRows(ids)
      const items: ListItem[] = filtered
        .filter(r => r.bestListId && r.bestPrice != null)
        .map(r => ({
          id: r.key,
          listId: r.bestListId!,
          originalName: r.internalName,
  ...
  return (
    <div className="animate-in">
      ...
      <p style={{ color: 'hsl(220 15% 45%)', fontSize: '0.875rem', marginBottom: '1.5rem' }}>
        Seleccioná 2 o 3 listas para comparar precios producto a producto
      </p>
      ...
      <div style={{ background: 'white', border: '1px solid hsl(220 15% 88%)', borderRadius: 12, padding: '1.25rem', marginBottom: '1.25rem' }}>
        ...
        {allLists.length === 0 ? (
          <p style={{ fontSize: '0.875rem', color: 'hsl(220 10% 55%)', padding: '0.5rem 0' }}>
            No hay listas disponibles. Importá una lista desde Excel o PDF primero.
          </p>
        ) : (
        ...
        <button
          ...
          style={{
            marginTop: '1rem', padding: '9px 20px',
      ...
      {/* Sin resultados tras comparar */}
      {!loading && rows.length === 0 && selectedIds.length >= 2 && (
        <div style={{
          background: 'hsl(37 91% 96%)', border: '1px solid hsl(37 91% 80%)',
          borderRadius: 10, padding: '1rem 1.25rem', fontSize: '0.875rem',
          color: 'hsl(37 91% 30%)', marginBottom: '1rem',
        }}>
          No se encontraron productos en común entre las listas seleccionadas. Los nombres deben coincidir para poder comparar.
        </div>
      )}
        ...
        <>
          <div style={{ display: 'flex', gap: 10, marginBottom: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: '0.8rem', color: 'hsl(220 10% 55%)' }}>
              {filtered.length} productos en común
            ...
            <table className="data-table">
              ...
              <tbody>
                {filtered.map(row => {
                  ...
                  return (
                    <tr key={row.key} style={{ background: checkedRows.has(row.key) ? 'hsl(221 89% 98%)' : undefined }}>
                      <td style={{ width: 40 }}>
                        <input type="checkbox"
                          checked={checkedRows.has(row.key)}
                      ...
                      {selectedLists.map(l => {
                        ...
                        const isBest = row.bestListId === l.id
                          ...
                          <td key={l.id} style={{
                            ...
                            {price != null ? (
                              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
                                {isBest && <span style={{ fontSize: '0.7rem', color: 'hsl(142 71% 35%)' }}>✓</span>}
                                {formatPrice(price)}
                              </span>
                            ) : (
                      ...
                      <td style={{ textAlign: 'right', fontWeight: 600, color: 'hsl(142 71% 32%)', fontFamily: 'var(--font-mono)' }}>
            ...
            {selectedLists.map(l => {
              ...
              return (
                <div key={l.id} style={{
                  ...
                  borderRadius: 10, padding: '0.875rem 1.25rem', flex: 1, minWidth: 180,
                }}>
